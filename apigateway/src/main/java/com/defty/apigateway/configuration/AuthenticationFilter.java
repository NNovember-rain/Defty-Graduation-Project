package com.defty.apigateway.configuration;

import com.defty.apigateway.dto.response.ApiResponse;
import com.defty.apigateway.service.IdentityService;
...
//        log.info("Token: {}", token);
        ...
        return identityService.verifyToken(token).flatMap(verifyResponse -> {
            if (verifyResponse.getResult().isValid())
                ...
                throwable -> unauthenticatedResponse(exchange.getResponse()));
    }

    @Override
    public int getOrder() {
        return -1; // TODO: Ensure this filter runs before others
    }
