...
export interface FormField {
    ...
    options?: { value: string | number; label: string; [key: string]: any }[]; // Sửa để support number
    required?: boolean;
    gridSpan?: number; // Cho layout grid
    format?: string;
    loading?: boolean; // Thêm để support loading state
    error?: string | null; // Thêm để support error state
    disabled?: boolean; // Thêm để support disabled state
}
...
const FormTemplate = <T extends Record<string, any>>({
    ...
    useEffect(() => {
        const loadData = async () => {
            if (isEditMode && serviceGetById && id && !initialData) {
                setLoading(true);
                setFetchError(null);
                try {
                    ...
                    setFetchError(t('formTemplate.fetchError', { message: error.message || '' }));
                } finally {
                    setLoading(false);
                }
            } else if (isEditMode && initialData) {
                // Sử dụng initialData nếu được cung cấp
        ...
        loadData();
    }, [isEditMode, id, initialData, serviceGetById, getInitialFormState, t]);
                                        ...
                                        <div className="form-template__select-wrapper">
                                            ...
                                            <select
                                                ...
                                                onChange={(e) => {
                                                    ...
                                                    // Chuyển đổi về number nếu giá trị trong options là number
                                                    const option = field.options?.find(opt => opt.value.toString() === value);
                                                    const finalValue = option && typeof option.value === 'number' ? Number(value) : value;
                                                    handleChange(field.key, finalValue);
                                                }}
                                                className="form-template__input form-template__select"
                                                disabled={loading || field.disabled || field.loading || !field.options?.length}
                                            >
                                                {/* Placeholder option */}
                                                <option value="" disabled>
                                                    {field.loading
                                                        ? t('common.loading')
                                                        : (field.placeholderKey ? t(field.placeholderKey) : t('common.pleaseSelect'))
                                                    }
                                                </option>
